...
public class RaycastExample : MonoBehaviour {
	...
	void Update () {
		...
		if( Physics.Raycast(transform.position, (target.transform.position -
			...
			Vector2 point= new Vector2(hit.point.x, hit.point.y);   //gives us hit location 
			point+=(new Vector2(hit.normal.x,hit.normal.y))*-0.5f;  //and mmoves it farther into block
